...
in {
  ...
  isUnlocked =
    "${pgrep} 'gpg-agent' &> /dev/null && ${gpg-connect-agent} 'scd getinfo card_list' /bye | ${grep} SERIALNO -q";
  lock = "${gpg-connect-agent} reloadagent /bye";
  unlock =
    "SSH_AUTH_SOCK=$(${gpgconf} --list-dirs agent-ssh-socket) ssh localhost exit";
}
